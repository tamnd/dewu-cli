...
describes what users get. Pick a format with `-o`, or let dewu choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dewu <command> -o table   # aligned columns for reading
dewu <command> -o jsonl   # one JSON object per line, for piping
dewu <command> -o json    # a single JSON array
dewu <command> -o csv     # spreadsheet friendly
dewu <command> -o tsv     # tab-separated
dewu <command> -o url     # just the URL column
dewu <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dewu <command> --fields id,title,url
```
...
```bash
dewu <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dewu <command>            # a table, because this is a terminal
dewu <command> | wc -l    # JSONL, because this is a pipe
```
